...
__kernel void	init_vals(__global t_data *data, __global t_val *vals)
{
	...
	l = get_global_size(0);
	vals[x + y * l].rc =
		(data->type == JULIA) ? data->jr : data->min_x +
		(data->max_x - data->min_x) * (double)x / (double)data->width;
	vals[x + y * l].ic =
		(data->type == JULIA) ? data->ji : data->max_y -
		(data->max_y - data->min_y) * (double)y / (double)data->height;
	vals[x + y * l].rz =
		(data->type == JULIA) ? data->min_x +
		(data->max_x - data->min_x) * (double)x / (double)data->width : 0;
	vals[x + y * l].iz =
		(data->type == JULIA) ? data->max_y -
		(data->max_y - data->min_y) * (double)y / (double)data->height : 0;
}
...
__kernel void	mandelbrot_julia(__global t_val *vals, __global int *buff,
